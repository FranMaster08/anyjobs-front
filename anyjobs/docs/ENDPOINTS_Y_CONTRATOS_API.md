...
- **Notificaciones**: `NOTIFICATIONS_API_URL` (por defecto: `…/notifications` vía same-origin / proxy)
...
- `NOTIFICATIONS_API_URL = https://<host>/notifications`
...
## Notificaciones (`NOTIFICATIONS_API_URL`)

Las peticiones contra `/notifications` reciben `Authorization: Bearer` vía interceptor si hay sesión. Solo el usuario autenticado puede leer o marcar sus propias notificaciones.

### GET `/` (`page`, `pageSize` opcionales)

**Auth**: Bearer (`notifications.read.own`).

**Response 200** — envoltorio paginado:
...
**Auth**: Bearer (`notifications.update.own`).

Marca una notificación del usuario como leída. **404** `NOTIFICATION.NOT_FOUND` si no existe o no pertenece al usuario.
...
### Creación automática (backend)

Al crear una propuesta exitosa (`POST /proposals`), el servidor crea una notificación `PROPOSAL_RECEIVED` para el `ownerUserId` de la solicitud (no para el postulante). La deduplicación usa `dedupKey` = `proposal:{proposalId}`.

### Retención de notificaciones leídas

Las notificaciones con `isRead: true` se eliminan automáticamente **24 horas después** de marcarse como leídas (según `updatedAt`). Las no leídas no expiran. La limpieza se ejecuta al listar o consultar el conteo de no leídas.
